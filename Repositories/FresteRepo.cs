...
public class FreteRepo
{
    ...
    public async Task AddFreteAsync(CadastroFrete frete)
    {
        using (var connection = new SqlConnection(_coneectionString)){
            connection.Open();
            var sql = "INSERT INTO CadastroFrete (Origem, Destino, Distancia, VeiculoId, Peso, Status, UsuarioId) VALUES (@Origem, @Destino, @Distancia, @VeiculoId, @Peso, @Status, @UsuarioId)";
